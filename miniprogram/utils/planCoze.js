...
function buildPlanCozePrompt(config, userProfile) {
  ...
  return (
    ...
    '必须只输出一个 JSON 对象，不要 markdown 说明，格式：\n' +
...
async function generatePlanWithCoze(config, user) {
  const prompt = buildPlanCozePrompt(config, user);
  try {
    ...
    console.warn('Coze plan fallback', e);
  }
  return { plan: { ...buildStudyPlan(config), source: 'template' }, cozeUsed: false };
}

module.exports = { generatePlanWithCoze };
