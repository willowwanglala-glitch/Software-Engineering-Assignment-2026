...
Page({
  ...
  async onLoad(options) {
    ...
    await Promise.all([this.loadHistory(), this.loadFaqSuggestions()]);
    if (options.q) {
  ...
  async loadFaqSuggestions() {
    try {
      const res = await api('listFaqs', {});
      const top = (res.list || []).slice(0, 6).map((f) => f.question);
      if (top.length) {
        this.setData({ suggestions: top.concat(['本周计划']) });
      }
    } catch (e) {
      console.warn('listFaqs', e);
    }
  },
  ...
  async sendMessage() {
    ...
    try {
      ...
      try {
        ...
        // 云函数未更新时也可能没有 hitTags，前端兜底写入本地 + 尝试同步
        const applied = await this.applyWeakPointsFromQuestion(text, logRes);
        if (applied && applied.hitTags.length) {
          const tagText = applied.hitTags.slice(0, 3).join('、');
          const tip =
            '已根据你的提问记录薄弱点：' +
            tagText +
            (applied.taskName
              ? '，并已加入学习任务「' + applied.taskName + '」。请返回首页查看「薄弱点强化」。'
              : '。请返回首页查看「薄弱点强化」。');
          this.setData({
            messages: this.data.messages.concat({ role: 'ai', content: tip })
          });
        }
      } catch (e) {
        console.warn('weakPoints', e);
      }
    } catch (e) {
      this.setData({
        messages: this.data.messages.concat({
  ...
  /**
   * 优先用接口返回；否则本地抽取并 updateProfile / addStudyTask
   */
  async applyWeakPointsFromQuestion(question, logRes) {
    let weakPoints = (logRes && logRes.weakPoints) || [];
    let hitTags = (logRes && logRes.hitTags) || [];
    let taskName = (logRes && logRes.suggestedTask && logRes.suggestedTask.name) || '';
    ...
    if (!hitTags.length) {
      ...
      weakPoints = mergeWeakPoints(local.weakPoints || [], hits);
      setLocalUser({ ...local, weakPoints });
      try {
        await api('updateProfile', { weakPoints });
      } catch (e) {
        console.warn('updateProfile weakPoints', e);
      }
      try {
        const marker = '【薄弱点·' + hits[0].name + '】';
        const taskRes = await api('listStudyTasks');
        const tasks = (taskRes && (taskRes.tasks || taskRes.list)) || [];
        const exists = tasks.some(
          (t) =>
            (t.status === 'todo' || t.status === 'doing') &&
            String(t.name || '').indexOf(marker) !== -1
        );
        if (!exists) {
          const addRes = await api('addStudyTask', {
            name: buildTaskName(hits[0]),
            type: hits[0].type || '薄弱点'
          });
          taskName = (addRes.task && addRes.task.name) || buildTaskName(hits[0]);
        } else {
          taskName = marker;
        }
      } catch (e) {
        console.warn('addStudyTask weak', e);
        taskName = buildTaskName(hits[0]);
      }
      return { hitTags, weakPoints, taskName };
    }
  ...
  reportAi() {
    wx.navigateTo({
      url: '/pages/feedback/feedback?from=ai'
    });
  },
