...
/** 篇幅上限：词数过少时硬性压分（大作文约 150+ 词，小作文约 80+ 词） */
...
function buildEssayReviewFromJson(json, essayContent, essayType, topic, rawFeedback) {
  ...
  if (words < 40) {
    // 极短文：维度也压低，避免「4 个词却各项 75」
    const low = dimensionBaseFromWords(words);
    return {
      score: guarded.score,
      wordCount: words,
      dimensions: defaultDimensions(low),
...
function buildEssayReviewResult(essayContent, cozeAnswer, essayType, topic) {
  ...
  const words = countWords(essayContent);
  let score = ruleScoreByWords(words, essayType);
  const base = dimensionBaseFromWords(words);
  const dimensions = defaultDimensions(base);
  ...
  if (text) {
    const lines = text
      ...
      .filter(Boolean);
    lines.forEach((line) => {
      if (/错误|问题|修改|应改|不一致|混淆|不足/i.test(line)) {
  ...
  if (!suggestions.success.length && words >= 80) {
    suggestions.success.push(
      essayType === 'small'
        ? '短文已具备基本格式，可继续打磨句式与语气。'
        : '文章已具备基本篇幅，可继续优化论证与衔接。'
    );
  }
...
function buildEssayCozePrompt(essayContent, essayType, topic) {
  ...
  return (
    ...
    '{"score":12,"dimensions":[{"name":"词汇","value":70},{"name":"语法","value":65},{"name":"结构","value":70},{"name":"内容","value":68},{"name":"逻辑","value":70}],"suggestions":{"error":["..."],"warning":["..."],"success":["..."]},"reference":"简短参考范文"}\n' +
    '评分规则：score 为 0-20 分；词数明显不足时必须给低分（少于 15 词不超过 4 分，少于 40 词不超过 6 分）；dimensions 为 0-100。\n' +
    '作文正文：\n' +
    essayContent
  );
}
